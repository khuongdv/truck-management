...
class AppSidebarNav extends Component {
  ...
  componentDidMount() {
    this.createData(this.props.navConfig);
  }

  createData(navConfig) {
    if (navConfig && navConfig.items.length > 0) {
      let pro = JSON.parse(JSON.stringify(navConfig.items))
      let ret = []
      pro.forEach((bigMenu) => {
        bigMenu.level = 1
        ret.push(bigMenu)
        if (bigMenu.children) {
          bigMenu.children.forEach(child => {
            child.level = 2
            ret.push(child)
            if (child.children) {
              child.children.forEach(chau => {
                chau.level = 3
                ret.push(chau)
              })
            }
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.navConfig && nextProps.navConfig.items && nextProps.navConfig.items.length > 0 && nextProps.navConfig !== this.props.navConfig) {
      this.createData(nextProps.navConfig);
    }
  }
    ...
    return (
      <li key={key} className={this.activeRoute(item.url, this.props)}>
        <a className="nav-link nav-dropdown-toggle" href="#" onClick={this.handleClick}><i
          className={classIcon}/>{item.name}{this.navBadge(item.badge)}</a>
  ...
  render() {
    ...
    return (
      <PerfectScrollbar className={navClasses} {...attributes} option={{suppressScrollX: !isRtl}}>
        <Nav>
          {children || this.navList(this.state.menuData.items)}
        </Nav>
      </PerfectScrollbar>
    );
  }
